...
*/
        ...
        List<PostListResponse> posts = switch (sortCriteria) {
            case "LATEST" -> postRepository.findByCaptionOrDecorationKeywordsOrderByLatest(request.keyword());
            case "POPULAR" -> postRepository.findByCaptionOrDecorationKeywordsOrderByPopular(request.keyword());
            default -> throw new IllegalArgumentException("유효하지 않은 정렬 기준입니다: " + sortCriteria);
        };
